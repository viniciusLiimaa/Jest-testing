"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.toPartiallyContain = toPartiallyContain;

var _jestMatcherUtils = require("jest-matcher-utils");

var _predicate = _interopRequireDefault(require("./predicate"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const passMessage = (actual, expected) => () => (0, _jestMatcherUtils.matcherHint)('.not.toPartiallyContain') + '\n\n' + 'Expected array not to partially contain:\n' + `  ${(0, _jestMatcherUtils.printExpected)(expected)}\n` + 'Received:\n' + `  ${(0, _jestMatcherUtils.printReceived)(actual)}`;

const failMessage = (actual, expected) => () => (0, _jestMatcherUtils.matcherHint)('.toPartiallyContain') + '\n\n' + 'Expected array to partially contain:\n' + `  ${(0, _jestMatcherUtils.printExpected)(expected)}\n` + 'Received:\n' + `  ${(0, _jestMatcherUtils.printReceived)(actual)}`;

function toPartiallyContain(actual, expected) {
  const pass = (0, _predicate.default)(actual, expected);

  if (pass) {
    return {
      pass: true,
      message: passMessage(actual, expected)
    };
  }

  return {
    pass: false,
    message: failMessage(actual, expected)
  };
}